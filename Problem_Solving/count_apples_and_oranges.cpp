...
void countApplesAndOranges(int s, int t, int a, int b, vector<int> apples, vector<int> oranges) {
    ...
    for (int dist : apples) {
        int landing_point = a + dist;
        if (landing_point >= s && landing_point <= t) count_apples++;
    }

    for (int dist : oranges) {
        int landing_point = b + dist;
        if  (landing_point >= s && landing_point <= t) count_oranges++;
    }
...
// if (find(sams_house.begin(), sams_house.end(), val) != sams_house.end()) ... To find if a particular element is in an array
